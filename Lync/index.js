...
try {
  ...
  if (fs.existsSync(CONFIG_PATH)) {
    ...
    for (const key in oldConfig)
      if (key in CONFIG) CONFIG[key] = oldConfig[key];
  }
...
┃      OPEN  ${cyan(
    "project.json",
    true
  )}?   Builds, syncs, and opens the project in Roblox Studio. ┃
┃      BUILD ${cyan(
    "project.json",
    true
  )}?   Builds the project to file.                            ┃
┃      FETCH ${cyan(
    "project.json",
    true
  )}?   Downloads the list of sources in the project file.     ┃
┃      GENERATE ${cyan(
    "project.json",
    true
  )}?   Generates the sourcemap.                               ┃
┣╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍┫
┃ ${cyan(
    "project.json",
    true
  )}?   The project file to read from and serve.                          ┃
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛ 
`);
...
if (MODE == "" || MODE == "help") {
  ...
  MODE == "config" ||
  ((MODE == "serve" || MODE == "open") &&
    PLATFORM == "linux" &&
    (CONFIG.Path_RobloxVersions == "" || CONFIG.Path_RobloxPlugins == ""))
) {
...
if (
  ...
  argHelp("Mode must be SERVE, OPEN, BUILD, or FETCH");
if (MODE == "open" && PLATFORM != "windows" && PLATFORM != "macos")
  argHelp("Cannot use OPEN mode on Linux");
const PROJECT_JSON =
  (ARGS[1] && ARGS[1].replace(/\\/g, "/")) || "default.project.json";
...
function localPathIsInit(localPath) {
  ...
  return (
    (localPathExt == ".lua" || localPathExt == ".luau") &&
    (localPathName == "init" || localPathName.endsWith(".init"))
  );
}
...
function assignMap(robloxPath, mapDetails, mtimeMs) {
  ...
  if (robloxPath in map.tree) {
    if (
      map.tree[robloxPath].Path != localPath &&
      !map.tree[robloxPath].ProjectJson
    ) {
  ...
  if (mapDetails.Meta)
    mTimes[mapDetails.Meta] = fs.statSync(mapDetails.Meta).mtimeMs; // Meta File stats are never retrieved before this, so they aren't in a function parameter
...
function mapLua(
  ...
  try {
    const directives = fs
      .readFileSync(localPath, { encoding: "utf-8" })
      .split(/[\r\n]+/);
      ...
      if (line.startsWith("--@")) {
        if (line.startsWith("--@script:")) {
          if (line == "--@script:legacy") {
            ...
            console.error(
              fileError(localPath),
              yellow("Invalid run context directive"),
              green(line)
            );
          }
        } else if (line == "--@script") {
          // Redundancy for '--@script:legacy'
          context = "Legacy";
        } else if (line == "--@localscript") {
          // Redundancy for '--@script:localscript'
          context = "LocalScript";
  ...
  if (context == "ModuleScript") {
    if ("Enabled" in properties) {
      ...
      console.error(
        ...
        yellow("Cannot use"),
        green("--@disabled"),
        yellow("directive on ModuleScripts")
      );
    }
  } else {
...
async function mapDirectory(localPath, robloxPath, flag) {
  ...
  if (localPathStats.isFile()) {
    ...
    if (flag != "Modified")
      robloxPath = robloxPathParsed.dir + "/" + robloxPathParsed.name;
    if (localPathExtensionIsMappable(localPath)) {
      ...
      if (
        ...
        let luaMeta;
        const metaLocalPathJson =
          localPath.slice(0, localPath.lastIndexOf("/")) +
          "/" +
          localPathParsed.name +
          ".meta.json";
        const metaLocalPathYaml =
          localPath.slice(0, localPath.lastIndexOf("/")) +
          "/" +
          localPathParsed.name +
          ".meta.yaml";
        const metaLocalPathToml =
          localPath.slice(0, localPath.lastIndexOf("/")) +
          "/" +
          localPathParsed.name +
          ".meta.toml";
        if (fs.existsSync(metaLocalPathJson)) {
          luaMeta = validateJson(
            "Meta",
            metaLocalPathJson,
            fs.readFileSync(metaLocalPathJson, { encoding: "utf-8" })
          );
          metaLocalPath = metaLocalPathJson;
        } else if (fs.existsSync(metaLocalPathYaml)) {
          luaMeta = validateYaml(
            "Meta",
            metaLocalPathYaml,
            fs.readFileSync(metaLocalPathYaml, { encoding: "utf-8" })
          );
          metaLocalPath = metaLocalPathYaml;
        } else if (fs.existsSync(metaLocalPathToml)) {
          luaMeta = validateToml(
            "Meta",
            metaLocalPathToml,
            fs.readFileSync(metaLocalPathToml, { encoding: "utf-8" })
          );
      ...
      if (localPathExt == ".lua" || localPathExt == ".luau") {
        ...
        if (flag != "JSON" && flag != "Modified")
          newRobloxPath = robloxPathParsed.dir + "/" + localPathParsed.name;
        mapLua(
          localPath,
          newRobloxPath,
          attributes,
        ...
        if (localPathName.endsWith(".project")) {
          mTimes[localPath] = localPathStats.mtimeMs;
          const subProjectJson = validateJson(
            "SubProject",
            localPath,
          ...
          if (
            ...
            assignMap(
              (flag != "Modified" && robloxPath.slice(0, -6)) || robloxPath,
              {
                Type: "JsonModel",
                Path: localPath,
              },
            ...
            assignMap(
              (flag != "Modified" && robloxPath.slice(0, -6)) || robloxPath,
              {
                ...
                Meta: path
                  .relative(
                    process.cwd(),
                    path.resolve(localPath, "..", excel.spreadsheet)
                  )
                  .replace(/\\/g, "/"),
              },
              localPathStats.mtimeMs
      ...
      console.error(
        fileError(localPath),
        yellow("File is not of a mappable file type")
      );
    }
    ...
    if (fs.existsSync(localPath + "/default.project.json")) {
      ...
      const subProjectJson = validateJson(
        "SubProject",
        subProjectJsonPath,
        fs.readFileSync(subProjectJsonPath, { encoding: "utf-8" })
      );
      if (subProjectJson) {
      ...
      if (fs.existsSync(localPath + "/" + localPathParentName + ".init.lua")) {
        mapLua(
          localPath + "/" + localPathParentName + ".init.lua",
          robloxPath,
          attributes,
          tags,
          undefined,
        ...
        fs.existsSync(localPath + "/" + localPathParentName + ".init.luau")
      ) {
        mapLua(
          localPath + "/" + localPathParentName + ".init.luau",
          robloxPath,
          attributes,
          tags,
          undefined,
 ...
 * @param {number} mtimeMs
 */
async function mapJsonRecursive(
  jsonPath,
  target,
  robloxPath,
  key,
  firstLoadingExternalPackage,
  externalPackageAppend,
  mtimeMs
) {
  ...
  if (externalPackageAppend && localPath)
    localPath = externalPackageAppend + localPath;
  ...
  assignMap(
    nextRobloxPath,
    {
      Type: "Instance",
      ClassName:
        (robloxPath == "tree" && key) || target[key]["$className"] || "Folder",
  ...
  for (const nextKey in target[key]) {
    if (
      nextKey[0] != "$" &&
      typeof target[key][nextKey] != "string" &&
      !Array.isArray(target[key][nextKey])
    ) {
      await mapJsonRecursive(
        jsonPath,
        target[key],
  ...
  if (localPath) {
    if (typeof localPath == "object") {
      ...
      if ("optional" in localPath) {
        ...
        const assetExt =
          (localPath.type != "repo" && "." + localPath.type) || "";
        try {
          if (
            !(
              fs.existsSync(assetFile + assetExt) || fs.existsSync(assetFile)
            ) ||
            tag == "latest"
          ) {
            // Get release info
            ...
            const release = await getAsync(
              ...
              }`,
              {
                ...
                Authorization:
                  ...
                  "Bearer " + CONFIG.GithubAccessToken,
            ...
            if (!fs.existsSync(assetFile + assetExt)) {
              ...
              if (localPath.type != "repo") {
                for (const assetInfo of release.assets) {
                  if (
                    "." + assetInfo.name.split(".").slice(-1)[0] ==
                    assetExt
                  ) {
                ...
                if (!matchingAssetInfo)
                  throw "No release assets matched type " + localPath.type;
              }
              ...
              if (localPath.type == "repo" || localPath.type == "zip") {
                const asset = await getAsync(
                  (localPath.type == "repo" &&
                    ...
                    `https://api.github.com/repos/${owner}/${repo}/releases/assets/${matchingAssetInfo.id}`,
                  {
                    Accept:
                      (localPath.type == "repo" &&
                ...
                await extractZIP(assetZip, {
                  dir: path.resolve(assetFile + "-unzipped"),
                });
                ...
                console.log(
                  `Downloaded ${green(localPath.package)} to ${cyan(assetFile)}`
                );
                ...
                console.log(
                  `Downloaded ${green(localPath.package)} to ${cyan(
                    assetFile + assetExt
                  )}`
                );
              }
...
async function changedJson() {
  ...
  projectJson = validateJson(
    "MainProject",
    PROJECT_JSON,
    fs.readFileSync(PROJECT_JSON, { encoding: "utf-8" })
  );
  if (!projectJson) {
    console.error(
      red("Terminated:"),
  ...
  let globIgnorePathsArr = [
    PROJECT_JSON,
    path
      .relative(process.cwd(), path.resolve(PROJECT_JSON, "../sourcemap.json"))
      .replace(/\\/g, "/"),
    "*.lock",
    ".git/*",
    "~$*",
  ...
  if (DEBUG)
    console.log(
      "Mapping",
      green(projectJson.name || path.parse(process.cwd()).name)
    );
  ...
  map.info.ContentRoot =
    path
      .join("lync", path.parse(process.cwd()).name, PROJECT_JSON)
      .replaceAll("\\", "/") + "/";
  if ("collisionGroups" in projectJson) {
    map.info.CollisionGroupsFile = projectJson.collisionGroups;
    map.info.CollisionGroups = JSON.parse(
      fs.readFileSync(projectJson.collisionGroups)
    );
  }
...
function runJobs(event, localPath) {
  if ("jobs" in projectJson) {
    for (const index in projectJson.jobs) {
      ...
      if (
        job.on.includes(event) &&
        picomatch(job.globPath)(localPath.replace(/\\/g, "/"))
      ) {
        ...
        if (job.commandName in CONFIG.JobCommands) {
          ...
          console.error(
            fileError(CONFIG_PATH),
            yellow("Missing job command"),
            green(job.commandName)
          );
        }
      }
    }
...
(async function () {
  ...
  if (MODE != "fetch" && CONFIG.AutoUpdate) {
    ...
    try {
      ...
      let latest = await getAsync(
        `https://api.github.com/repos/${CONFIG.AutoUpdate_Repo}/releases${
          (!CONFIG.AutoUpdate_UsePrereleases && "/latest") || ""
        }`,
        {
          Accept: "application/vnd.github+json",
          Authorization:
      ...
      if (!latest || !("id" in latest))
        throw "Failed to get update release info";

      if (latest.id != CONFIG.AutoUpdate_LatestId) {
        const updateFile = path.resolve(
          LYNC_INSTALL_DIR,
          `Lync-${latest.tag_name}.zip`
        );
        const updateFolder = path.resolve(
          LYNC_INSTALL_DIR,
          "Lync-" + latest.tag_name
        );
        ...
        if (!assetId)
          throw `Failed to find update release asset with name '${assetName}'`;
        const update = await getAsync(
          `https://api.github.com/repos/${CONFIG.AutoUpdate_Repo}/releases/assets/${assetId}`,
          {
            Accept: "application/octet-stream",
            Authorization:
              CONFIG.GithubAccessToken != "" &&
        ...
        if (UTF8.decode(update.subarray(0, 2)) != "PK")
          throw "Failed to download update release asset";
        ...
        if (PLATFORM == "windows") {
          fs.renameSync(
            path.resolve(updateFolder, "lync.exe"),
            path.resolve(LYNC_INSTALL_DIR, path.parse(executable).base)
          );
        } else {
          fs.renameSync(
            path.resolve(updateFolder, "lync"),
            path.resolve(LYNC_INSTALL_DIR, path.parse(executable).base)
          );
        }
  ...
  if ((MODE == "open" && !("base" in projectJson)) || MODE == "build") {
    ...
    const lunePath =
      (PLATFORM == "windows" &&
        CONFIG.Path_Lune.replace("%LOCALAPPDATA%", process.env.LOCALAPPDATA)) ||
      (PLATFORM == "macos" &&
        CONFIG.Path_Lune.replace("$HOME", process.env.HOME)) ||
      CONFIG.Path_Lune;
    ...
    function toEscapeSequence(str) {
      let escapeSequence = "";
      let i = str.length;
      while (i--) escapeSequence = "\\" + str.charCodeAt(i) + escapeSequence;
    ...
    function mapProperties(properties) {
      for (const property in properties) {
        ...
        if (Array.isArray(value) && !(value in loadstringMapEntries)) {
          loadstringMap += `\t[ "${toEscapeSequence(
            "return " + value
          )}" ] = ${value};\n`;
    ...
    for (const key in map.tree) {
      ...
      if (mapping.Type == "JsonModel") {
        ...
        const jsonModel = validateJson(
          "Model",
          mapping.Path,
          fs.readFileSync(mapping.Path, { encoding: "utf-8" })
        );
        if (jsonModel) mapJsonModel(jsonModel);
      } else if ("Properties" in mapping) mapProperties(mapping.Properties);
      if ("TerrainMaterialColors" in mapping)
        mapProperties(mapping.TerrainMaterialColors);
    }

    // Fetch script functions
    let pluginSource = fs.readFileSync(
      path.resolve(__dirname, "RobloxPluginSource/Plugin.lua"),
      { encoding: "utf8" }
    );
    pluginSource = pluginSource.substring(
      pluginSource.indexOf("--offline-start") + 15,
      pluginSource.indexOf("--offline-end")
    );
    ...
    if (fs.existsSync(buildScriptPath)) fs.rmSync(buildScriptPath);
    ...
    if (validationStatus == null) {
      console.error(
        red("Build error:"),
        yellow("Lune executable not found:"),
        cyan(lunePath)
      );
      process.exit(1);
      ...
      console.error(
        red("Build error:"),
        yellow("Validation script failed with status", validationStatus)
      );
      process.exit(2);
    }
    ...
    let buildScript =
      ...
      `${pluginSource}\n` +
      `map = net.jsonDecode("${toEscapeSequence(
    ...
    build.on("close", (status) => {
      if (status == null) {
        ...
        console.error(
          red("Build error:"),
          yellow("Build script failed with status"),
          status
        );
        process.exit(3);
      }
  ...
  if (MODE == "open" && "base" in projectJson) {
    if (DEBUG)
      console.log(
        "Copying",
        cyan(projectJson.base),
        "->",
        cyan(projectJson.build)
      );
    ...
    if (PLATFORM == "windows" || PLATFORM == "macos") {
      if (DEBUG) console.log("Opening", cyan(projectJson.build));
      spawn(
        ((PLATFORM == "macos" && "open -n ") || "") + `"${projectJson.build}"`,
        [],
        {
          stdio: "ignore",
          detached: true,
  ...
  // Sync
  if (MODE == "serve" || MODE == "open" || MODE == "generate") {
    // Copy plugin
    const pluginsPath = path.resolve(
      (PLATFORM == "windows" &&
        CONFIG.Path_RobloxPlugins.replace(
          "%LOCALAPPDATA%",
          process.env.LOCALAPPDATA
        )) ||
        (PLATFORM == "macos" &&
          CONFIG.Path_RobloxPlugins.replace("$HOME", process.env.HOME)) ||
        CONFIG.Path_RobloxPlugins
    );
    ...
    const newHash = crypto
      .createHash("md5")
      .update(fs.readFileSync(path.resolve(__dirname, "Plugin.rbxm")))
      .digest("hex");
    if (currentHash != newHash) {
      if (DEBUG)
        console.log(
          "Copying",
          cyan(path.resolve(__dirname, "Plugin.rbxm")),
          "->",
          cyan(pluginPath)
        );
                ...
                if (localPath in mTimes) {
                  ...
                  if (!localPathStats) {
                    ...
                    for (const key in map.tree) {
                      // Direct, non-Init
                      if (
                        map.tree[key].Path == localPath &&
                        map.tree[key].InitParent != parentPathString
                      ) {
                      ...
                      if (
                        key in map.tree &&
                        map.tree[key].Meta &&
                        (map.tree[key].Meta == localPath ||
                          map.tree[key].Meta.startsWith(localPath + "/"))
                      ) {
                      ...
                      if (
                        key in map.tree &&
                        map.tree[key].ProjectJson == localPath
                      ) {
                        if (fs.existsSync(map.tree[key].Path)) {
                          await mapDirectory(
                            map.tree[key].Path,
                  ...
                  // Added
                  if (
                    parentPathString in mTimes &&
                    (!localPathStats.isFile() ||
                      localPathExtensionIsMappable(localPath))
                  ) {
                    console.log("A", cyan(localPath));
                    for (const key in map.tree) {
                      if (
                        map.tree[key].Path == parentPathString ||
                        map.tree[key].InitParent == parentPathString
                      ) {
                        const localPathParsed = path.parse(localPath);
                        const localPathName =
                          localPathParsed.name.toLowerCase();
                        const localPathExt = localPathParsed.ext.toLowerCase();

                        // Remap adjacent matching file
                        if (
                          localPathName != "init.meta" &&
                          localPathName.endsWith(".meta") &&
                          (localPathExt == ".json" ||
                            localPathExt == ".yaml" ||
                            localPathExt == ".toml")
                        ) {
                          ...
                          if (
                            ...
                            fs.existsSync(
                              localPathParsed.dir + "/" + title + ".luau"
                            )
                          ) {
                            delete map.tree[key];
                          ...
                          (localPathName == "init.meta" &&
                            (localPathExt == ".json" ||
                              localPathExt == ".yaml" ||
                              localPathExt == ".toml")) ||
                          localPathParsed.base == "default.project.json"
                        ) {
                          delete map.tree[key];
                          await mapDirectory(parentPathString, key);
